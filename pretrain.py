...
def main():
    ...
    model.init(eMat=eMat, rMat=rMat)
    ...
    for epoch_idx in range(config.trainTimes):
        ...
        with torch.no_grad():
            ...
            if epoch_idx % config.test_period == 0:
                ...
                if config.save_flag and epoch_idx >= 20 and epoch_idx%20 == 0:
                    save_dict = {"entMatrix": model.ent_embed.weight.data.cpu().numpy(),
                                 "relMatrix": model.rel_embed.weight.data.cpu().numpy(),
                                 "neiMatrix": model.nei_embed.weight.data.cpu().numpy()}
                    matrixSave(save_dict, config, epoch_idx, metric)
            if epoch_idx % 20 == 0:
                print("whole test")
